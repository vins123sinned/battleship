...
export function takeAdjacent(row, column, takenCoordinates) {
  ...
  adjacentCells.forEach((cell) => {
    ...
    if (!takenCoordinates.has(`${cell[0]},${cell[1]}`))
      takenCoordinates.add(`${cell[0]},${cell[1]}`);
  });
}
...
export function temporaryAdjacents(row, column, skipCells) {
  ...
  adjacentCells.forEach((cell) => {
    ...
    if (!temporaryCoordinates.has(`${cell[0]},${cell[1]}`))
      temporaryCoordinates.add(`${cell[0]},${cell[1]}`);
  });
}
...
function adjacentsTaken(row, column, skipCells) {
  ...
  const playerGameboard = document.querySelector(
    `[data-player="${dragInfo.player.name}"]`,
  );
...
export function getStartingCoords(coordinate) {
  ...
  const isVertical =
    draggedShipInstance.direction === "vertical" ? true : false;
...
export function switchIsPossible() {
  ...
  const [startingRow, startingColumn] = startingCoordinate
    .split(",")
    .map(Number);
  const isVertical =
    draggedShipInstance.direction === "vertical" ? false : true;
  let valid = true;
...
export function takeDiagonalCoordinates(currentBoard, coordinate, computer) {
  ...
  diagonalCoords.forEach((coord) => {
    ...
    if (
      currentBoard.attacks.find((attack) => attack.coordinate === `${r},${c}`)
    )
      return;
...
function chooseComputerCoordinates(playerOne) {
  ...
  for (const cell of hitCells) {
    ...
    for (const [r, c] of adjacentCells) {
      ...
      if (
        !playerOne.gameboard.isAlreadyAttacked(`${r},${c}`) &&
        playerOne.gameboard.availableMoves.has(`${r},${c}`)
      ) {
