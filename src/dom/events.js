...
import {
  ...
  showPassScreen,
  removePassScreen,
  removeIntermissionDiv,
  showEndTurnButton,
} from "./dom";
import {
  computerAttacks,
  getStartingCoords,
  getStartingSwitchCoords,
  switchIsPossible,
  takeAdjacent,
...
export function shipMousedown(event, ship) {
  dragInfo.player =
    event.target.parentNode.parentNode.dataset.player === "Player One"
      ? players.playerOne
      : players.playerTwo;
...
function getCurrentCoordinate(event) {
  ...
  const findElement = elementsAtPoint.find((element) =>
...
export function switchShipDirection(event) {
  ...
  if (switchIsPossible()) {
    ...
    draggedShipInstance.direction =
      draggedShipInstance.direction === "vertical" ? "horizontal" : "vertical";
...
export function switchTurns() {
  ...
  const newCurrentPlayer =
    players.currentPlayer === playerOne ? playerTwo : playerOne;
